        ...
        <div class="flex justify-center">
          <div class="w-10/12">
            <template v-if="state.category?.numberOfArtworks">
              ...
              <AtomsObserver
                v-show="hasMore"
                class="flex justify-center items-center h-10 mt-3"
                threshold="0.75"
                @intersect="loadMore"
              >
...
state.artworks = await listOrFetchByCategory(
  category,
  state.pageSize,
  state.page,
);
...
const loadMore = async () => {
  state.page += 1;
  state.artworks.push(
    ...(await listOrFetchByCategory(category, state.pageSize, state.page)),
  );
};
</script>
